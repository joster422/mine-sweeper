...
export class SweeperService {
  ...
  async getMoveAsync(grid: Cell[][]) {
    ...
    for (let x = 0; x < grid.length; x++) {
      for (let y = 0; y < grid[x].length; y++) {
        ...
        if (mineNeighbors.length && mineNeighbors.length == cell.score) {
          let safe = hiddenNeighbors.filter(
            neighbor => mineNeighbors.indexOf(neighbor) == -1
          );
    ...
    for (let x = 0; x < grid.length; x++) {
      for (let y = 0; y < grid[x].length; y++) {
        ...
        combinations = combinations.filter(
          x => x.length != hiddenNeighbors.length
        );

        for (let i = 0; i < combinations.length; i++) {
          let combination = combinations[i];
          let total = combination.reduce(
            (acc, val) => acc + val.probability,
            0
          );
          ...
          let timeout =
            combinations.length > 16 ? this.timeout / 7 : this.timeout;
          ...
          let safe = hiddenNeighbors.filter(
            neighbor => combination.indexOf(neighbor) == -1
          );
    ...
    let guess = grid.reduce(
      (acc, val) =>
        acc.concat(val.filter(cell => cell.hidden && cell.probability != 1)),
      []
    );
  ...
  private getNeighbors(cell: Cell, grid: Cell[][]): Cell[] {
    ...
    for (let i = -1; i <= 1; i++) {
      for (let j = -1; j <= 1; j++) {
        (i != 0 || j != 0) &&
          grid[cell.x + i] !== undefined &&
          grid[cell.x + i][cell.y + j] !== undefined &&
          ret.push(grid[cell.x + i][cell.y + j]);
      }
    }
    return ret;
  }
